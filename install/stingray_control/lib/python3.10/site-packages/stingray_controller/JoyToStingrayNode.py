...
class JoyToStingrayNode(Node):
    def __init__(self):
        ...
        self.light_level = 0  # Initialize light level

    def joy_callback(self, msg):
        # Xbox one controller configuration:

        # Mapping for joystick and dpad axes:
        forward_backward = self.scale_input(msg.axes[1], -1.0, 1.0, 1100, 1900)  # Left stick Y-axis
        left_right = self.scale_input(msg.axes[0], -1.0, 1.0, 1100, 1900)  # Left stick X-axis
        increase_altitude = self.scale_input(msg.axes[5], 1.0, -1.0, 1500, 1900)
        decrease_altitude = self.scale_input(msg.axes[2], 1.0, -1.0, 1500, 1100)
        horizontal_rotation = self.scale_input(msg.axes[3], -1.0, 1.0, 1100, 1900)  # Right stick X-axis
        vertical_rotation = self.scale_input(msg.axes[4], -1.0, 1.0, 1100, 1900)  # Right stick Y-axis

        camera_angle = int(self.scale_input(msg.axes[7], -1, 1, 1300, 1700)) #Up/DOwn on dpad maxMin values = (1100 and 1900)

        #camera_angle = msg.axes[7]  # up/down  dpad
      ...
      #Increase light brightness by 1 when increase or decrease is pressed
        if light_brighter and not self.prev_light_brighter and self.light_level < 8:
            self.light_level += 1

        if light_dimmer and not self.prev_light_dimmer and self.light_level > 0:
            self.light_level -= 1
